...
All contract documentation lives at the repository root:
...
`lib/` follows a feature-first structure (`auth`, `dashboard`, `candidates`, `registration`).
Auth, registration, dashboard, and candidate browsing are implemented against the Laravel API.
Ballot, voting, results, and candidacy surfaces are **not yet scaffolded** — they will be added
together with the corresponding backend endpoints defined in `../docs/05_API_INTEGRATION.md`.
See [`../docs/AUDIT.md`](../docs/AUDIT.md) for the latest cross-codebase audit and cleanup record.
